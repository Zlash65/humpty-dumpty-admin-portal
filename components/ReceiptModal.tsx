...
const normalizeReceipt = (raw: string | undefined): string => {
    if (!raw) return '';
    const s = String(raw);
...
const toMonthName = (val: string | undefined): string | null => {
    if (!val) return null;
...
const displayUptoMonth = (monthYear: string | undefined): string => {
...
const Underline = ({ children, width = 200, align = 'left' }: UnderlineProps) => (
    <Box
        sx={{
            borderBottom: '0 none',
            minWidth: width,
...
// Single receipt copy
const ReceiptCopy = ({ data, settings }: ReceiptCopyProps) => {
...
export default function ReceiptModal({ open, onClose, transaction, student, settings }: ReceiptModalProps) {
            ...
            <GlobalStyles
                styles={`
                    @media print {
                        ...
                        .receipt-copy-2 { display: block !important; margin-top: 8mm; }
                        .receipt { page-break-inside: avoid; margin: 0 auto 8mm; }
                    }
                    @media screen {
