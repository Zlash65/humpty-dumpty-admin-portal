...
function normalizeClassesColumnsModel(model) {
    ...
    useEffect(() => {
        (async () => {
            try {
                ...
                const model = normalizeClassesColumnsModel(saved?.columnVisibilityModel);
...
function ClassEntryDialog({ mode, open, onClose, academicYearId, branchId, initial, onDone }) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const msg = err?.message || 'Failed to save class';
            setError(msg);
            onDone?.({ error: msg });
        } finally {
            setSubmitting(false);
