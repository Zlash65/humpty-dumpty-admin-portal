...
function normalizeStudentColumnsModel(model) {
    ...
    useEffect(() => {
        (async () => {
            try {
                ...
                const model = normalizeStudentColumnsModel(saved?.columnVisibilityModel);
...
function AddOrEditStudentDialog({ mode, open, onClose, academicYearId, branchId, classEntries, initial, onDone }) {
    ...
    const ensureRollNumber = async (nextClassKey, nextSection) => {
